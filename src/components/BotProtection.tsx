...
export const BotProtectionModal: React.FC<BotProtectionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[999990] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="max-w-md w-full bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-7 shadow-2xl space-y-5 text-black dark:text-white relative">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-zinc-800">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-extrabold text-sm text-black dark:text-white">Cloudflare &amp; reCAPTCHA</h3>
              <p className="text-[10px] text-black dark:text-zinc-400 font-medium">বট ও স্প্যাম প্রতিরোধ ব্যবস্থা (DDoS &amp; Bot Protection)</p>
            </div>
          </div>
          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-black text-white font-bold border border-zinc-700">
            TLS 1.3
          </span>
        </div>
        ...
        <div className="text-center space-y-1.5 pt-1">
          ...
          <p className="text-xs text-black dark:text-zinc-300 font-medium">
            নিশ্চিত করুন যে আপনি কোনো স্বয়ংক্রিয় রোবট বা স্ক্রিপ্ট নন।
          </p>
        </div>

        {/* Cloudflare Turnstile Styled Interactive Box */}
        <div className="p-4 rounded-2xl bg-black text-white border-2 border-zinc-800 flex items-center justify-between shadow-sm">
          <div
            ...
            <div
              className={`w-7 h-7 rounded-lg border-2 flex items-center justify-center transition-all ${
                status === 'success'
                  ...
                  : 'border-zinc-500 hover:border-white bg-zinc-900'
              }`}
            >
              {status === 'success' && <Check className="w-4 h-4 stroke-[3] text-black" />}
              {status === 'verifying' && <RefreshCw className="w-4 h-4 text-white animate-spin" />}
            </div>

            <span className="text-xs font-bold text-white">
              {status === 'success' ? (
                <span className="text-white">যাচাই সফল হয়েছে (Verified Human)</span>
              ) : status === 'verifying' ? (
                <span className="text-zinc-300 animate-pulse">যাচাই করা হচ্ছে...</span>
              ) : (
          ...
          <div className="flex flex-col items-end pl-2 text-right">
            <div className="flex items-center gap-1">
              <svg className="w-6 h-4 text-white" viewBox="0 0 100 40" fill="currentColor">
              ...
              <span className="text-[10px] font-black text-white tracking-tight">CLOUDFLARE</span>
            </div>
        ...
        <div className="pt-2 flex items-center justify-between text-[10px] text-black dark:text-zinc-400 font-mono border-t border-slate-200 dark:border-zinc-800 font-medium">
          <span>Ray ID: {rayId}</span>
          <span className="flex items-center gap-1 text-black dark:text-zinc-200 font-sans font-bold">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="w-full py-2 text-xs font-bold text-black dark:text-white hover:underline text-center cursor-pointer"
          >
            বাতিল করুন
          </button>
        )}
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-200 dark:border-slate-800 shadow-md text-slate-700 dark:text-slate-300 text-[10px] font-medium hover:border-slate-400 transition-all cursor-pointer"
      ...
      >
        ...
        <span className="font-bold text-slate-900 dark:text-white">Cloudflare</span>
      ...
      {showDetails && (
        <div className="absolute bottom-10 right-0 w-64 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl text-xs space-y-2 text-slate-800 dark:text-slate-200 animate-fade-in">
          <div className="flex items-center justify-between font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-1.5">
            ...
            <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono">ACTIVE</span>
          </div>
          ...
          <div className="text-[9px] text-slate-400 font-mono pt-1 border-t border-slate-100 dark:border-slate-800 flex justify-between">
            ...
            <button onClick={() => setShowDetails(false)} className="text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white font-bold cursor-pointer">বন্ধ</button>
          </div>
        </div>
      )}
    </div>
